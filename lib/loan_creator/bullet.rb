module LoanCreator
  class Bullet < LoanCreator::Common
    ...
    def compute_last_term
      ...
      @period_interests                   = @due_interests_end_of_period + compute_capitalized_interests(@due_on)
    ...
    def compute_capitalized_interests(due_date)
      (amount + @due_interests_beginning_of_period).mult(periodic_interests_rate(due_date), BIG_DECIMAL_DIGITS)
    end

    def compute_term
      @due_interests_beginning_of_period = @due_interests_end_of_period
      @due_interests_end_of_period += compute_capitalized_interests(@due_on)
    end
  end
end
